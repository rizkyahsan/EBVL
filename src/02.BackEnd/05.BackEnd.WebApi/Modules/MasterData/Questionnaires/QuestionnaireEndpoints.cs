...
public sealed class QuestionnaireEndpoints : IEndpoint
{
    public RouteHandlerBuilder RegisterTo(WebApplication app)
    {
        ...
        _ = app.MapPut(QuestionnaireRoutes.UpdateSection, async (Guid questionnaireId, Guid sectionId, AddQuestionnaireRequest body, ISender sender, CancellationToken ct) => Results.Ok(await sender.Send(new UpdateQuestionnaireSectionCommand(questionnaireId, sectionId, body), ct))).AllowAnonymous().WithTags(RouteConfig.Tag).WithName("Questionnaires.UpdateSection");
