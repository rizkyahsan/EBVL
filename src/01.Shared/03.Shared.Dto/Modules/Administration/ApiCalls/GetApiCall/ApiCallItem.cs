namespace EBVL.Shared.Dto.Modules.Administration.ApiCalls.GetApiCall;

public sealed record ApiCallItem
{
    public required Guid Id { get; init; }
    public required DateTimeOffset Created { get; init; }
    public required string CreatedBy { get; init; }

    public required string ServiceName { get; init; }
    public required string ServiceProvider { get; init; }
    public required string ServiceCategory { get; init; }
    public required string RequestUrl { get; init; }
    public required string RequestMethod { get; init; }
    public required IEnumerable<KeyValuePair<string, string>> RequestParameters { get; init; }
    public required ushort ResponseStatusCode { get; init; }
    public required IEnumerable<KeyValuePair<string, string>> ResponseHeaders { get; init; }
    public required string? ResponseContent { get; init; }
    public required string? ErrorMessage { get; init; }
}
